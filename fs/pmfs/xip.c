          ...
          size_t count, loff_t pos, loff_t *ppos)
{
	...
	do {
	    ...
	    while (remain != 0) {
	        ...
	        chunk_offset = (unsigned long) iter_xmem &
	                (PMFS_MEMCPY_CHUNK_SIZE - 1);

	        /*to_copy: how many bytes to copy to reach the end of the block */
	        to_copy = PMFS_MEMCPY_CHUNK_SIZE - chunk_offset;
	        if (to_copy > remain)
	            to_copy = remain;

	        adv = memcpy_to_nvmm((char *)iter_xmem, 0, iter_buf, to_copy);
	...
	loff_t *ppos, u64 block)
{
	...
	size_t copied, ret = 0, offset, to_copy;
	size_t chunk_offset;
	timing_t memcpy_time;
	void *iter_xmem, *iter_buf;
	loff_t pos_iter;
	...
	while (count != 0) {
	    ...
	    chunk_offset = pos_iter & (PMFS_MEMCPY_CHUNK_SIZE - 1);
	    /*to_copy: how many bytes to copy to reach the end of the block */
	    to_copy = PMFS_MEMCPY_CHUNK_SIZE - chunk_offset;
	    if (to_copy > count)
	        to_copy = count;
	...
	bool is_end_blk)
{
	...
	if (new_blk) {
		...
		if (ptr != NULL) {
			...
			while (count > 0) {
			    size_t to_memset;
			    offset = (unsigned long) ptr_iter & (PMFS_MEMCPY_CHUNK_SIZE - 1);
			    to_memset = PMFS_MEMCPY_CHUNK_SIZE - offset;

			    if (to_memset > count)
			        to_memset = count;

			    memset_nt(ptr_iter, 0, to_memset);
