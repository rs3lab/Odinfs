 ...
 * with d_instantiate().
 */
static int nova_create(struct user_namespace *mnt_userns,
...
static int nova_mknod(struct user_namespace *mnt_userns,
		      struct inode *dir, struct dentry *dentry,
...
static int nova_symlink(struct user_namespace *mnt_userns,
			struct inode *dir, struct dentry *dentry,
			const char *symname)
{
	struct super_block *sb = dir->i_sb;
...
static int nova_mkdir(struct user_namespace *mnt_userns,
