	...
	size_t offset, size_t length, void *kmem)
{
	...
	if (ptr != NULL) {
		if (support_clwb) {
			if (memcpy(kmem + offset, ptr + offset, length) == NULL)
				rc = -1;
		} else
	...
	int allocated, u64 begin_tail, u64 end_tail)
{
	...
	while (curr_p != end_tail) {
		...
		if (metadata_csum == 0)
			 ...
			 */
			if (memcpy(entryc, entry,
				   sizeof(struct nova_file_write_entry)) == NULL)
				return -EIO;
		}
	...
	bool inplace)
{
	...
	if (offset != 0) {
		...
		if (entry != NULL) {
			...
			if (memcpy(blockbuf, blockptr, offset) == NULL)
				ret = -1;
			if (ret < 0)
				goto out;
		} else {
...
eblk:
	...
	if (eblk_offset != 0) {
		...
		if (entry != NULL) {
			...
			if (memcpy(blockbuf + eblk_offset,
				   blockptr + eblk_offset,
				   blocksize - eblk_offset) == NULL)
				ret = -1;
			if (ret < 0)
				goto out;
		} else {
...
//		nova_dbg("Write: %p\n", kmem);
		...
		kmem_iter = kmem + offset;
		buf_iter = (char *) buf;
		remain = bytes;

		while (remain > 0) {
		    size_t to_copy = NOVA_MEMCPY_CHUNK_SIZE;
		    size_t ret, page_offset, delta;

		    page_offset = ((unsigned long) kmem_iter) & (to_copy -1);
...
#if NOVA_MEMCPY_NT
		    ret = memcpy_to_pmem_nocache(kmem_iter, buf_iter, to_copy);
#else
		    ret = memcpy_to_pmem_flush(kmem_iter, buf_iter, to_copy);
#endif
...
static int nova_iomap_begin_lock(struct inode *inode, loff_t offset,
	loff_t length, unsigned flags, struct iomap *iomap, struct iomap *srcmap)
{
