...
void nova_sysfs_init(struct super_block *sb)
{
	...
	if (sbi->s_proc) {
		ops = &nova_seq_timing_fops;
		proc_create_data("timing_stats", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_IO_fops;
		proc_create_data("IO_stats", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_allocator_fops;
		proc_create_data("allocator", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_create_snapshot_fops;
		proc_create_data("create_snapshot", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_delete_snapshot_fops;
		proc_create_data("delete_snapshot", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_show_snapshots_fops;
		proc_create_data("snapshots", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_test_perf_fops;
		proc_create_data("test_perf", 0444, sbi->s_proc, ops, sb);

		ops = &nova_seq_gc_fops;
		proc_create_data("gc", 0444, sbi->s_proc, ops, sb);
	}
}
