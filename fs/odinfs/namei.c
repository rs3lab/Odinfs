		       ...
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	PMFS_DEFINE_TIMING_VAR(create_time);
	PMFS_DEFINE_TIMING_VAR(new_inode_time);
	PMFS_DEFINE_TIMING_VAR(add_nondir_time);
	PMFS_DEFINE_TIMING_VAR(new_trans_time);
	PMFS_DEFINE_TIMING_VAR(commit_trans_time);
...
static int pmfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	PMFS_DEFINE_TIMING_VAR(unlink_time);
		      ...
		      struct dentry *dentry, umode_t mode)
{
	 ...
	 */
	err = pmfs_alloc_blocks(NULL, inode, 0, 1, 0);
	if (err)
		goto out_clear_inode;
	inode->i_size = sb->s_blocksize;

	bp = pmfs_find_data_block(inode, 0);
