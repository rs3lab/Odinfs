...
/* XXX: INODE_TABLE_START and INODE_TABLE_NUM_BLOCKS are inherited from WineFS */
/* This hard wired value is so bad. Fix it when have time */
...
#define PMFS_DIR_REC_LEN(name_len)                                             \
	(((name_len) + 12 + PMFS_DIR_ROUND) & ~PMFS_DIR_ROUND)

/* PMFS supported data blocks */
#define PMFS_BLOCK_TYPE_4K 0
#define PMFS_BLOCK_TYPE_32K 1
#define PMFS_BLOCK_TYPE_2M 2
#define PMFS_BLOCK_TYPE_1G 3
#define PMFS_BLOCK_TYPE_MAX 4
 ...
 */
#define PMFS_DEFAULT_BLOCK_TYPE PMFS_BLOCK_TYPE_32K
...
struct pmfs_inode {
	...
	__le32 nsocket; /* the next socket to allocate pages from */
	/* remember to align this data structure */
};
...
/* Inherits from WineFS, looks a value that does not make much sense */
