...
module_param(measure_timing_pmfs, int, S_IRUGO);
MODULE_PARM_DESC(measure_timing_pmfs, "Timing measurement");
...
struct kmem_cache *pmfs_range_node_cachep;
struct kmem_cache *pmfs_rangelock_cachep;

#ifdef GLOBAL_RGLOCK_BRAVO
volatile atomic_t **global_vr_table;
#endif
...
static int pmfs_get_block_info(struct super_block *sb, struct pmfs_sb_info *sbi)
{
	int i;

	if (pmem_ar_dev.elem_num == 0) {
		pmfs_err(sb, "No device in pmem array!\n");
		return -EINVAL;
	}
   ...
   */
	for (i = 0; i < pmem_ar_dev.elem_num; i++) {
		...
		ret = bdev_dax_supported(bdev, PAGE_SIZE);
		if (!ret) {
			pmfs_err(sb, "device does not support DAX\n");
			return -EINVAL;
		}

		/* FIXME: pmfs does not do this but I think maybe we need to put
      these devs */
		...
		if (i == 0 || virt_addr < sbi->start_virt_addr) {
			sbi->start_virt_addr = virt_addr;
	...
	for (i = 0; i < pmem_ar_dev.elem_num; i++) {
		/*
     * Implicitly assume 4KB block size is bad, but it is all over pmfs
     * code ...
     */

		unsigned long size_in_blocks =
			pmem_ar_dev.size_in_bytes[i] >> PAGE_SHIFT;
     ...
     */
		...
		sbi->block_info[i].start_block =
			(pmem_ar_dev.start_virt_addr[i] -
			 (unsigned long)sbi->start_virt_addr) >>
			PAGE_SHIFT;
		...
		pmfs_dbg("heap socket: %d, start_block: %lu, end_block: %lu\n",
...
static struct pmfs_inode *pmfs_init(struct super_block *sb, unsigned long size)
{
...
#ifdef CONFIG_PMFS_TEST
	if (!first_pmfs_super)
		first_pmfs_super = sbi->start_virt_addr;
#endif
	...
	pmfs_dbg_verbose("journal meta start %llx data start 0x%llx, "
			 ...
			 journal_meta_start, journal_data_start, sbi->jsize,
			 inode_table_start);
	...
	sbi->head_reserved_blocks =
		((journal_data_start + sbi->jsize) + sb->s_blocksize - 1) >>
		sb->s_blocksize_bits;
	...
	pmfs_new_index_blocks(sb, root_i, &blocknr, 1);
	...
	de = (struct pmfs_direntry *)pmfs_get_virt_addr_from_offset(
		sb, pmfs_get_block_off(sb, blocknr, PMFS_BLOCK_TYPE_4K));
...
static inline void set_default_opts(struct pmfs_sb_info *sbi)
{
	...
	/* See comments in pmfs_get_block_info */
...
int pmfs_check_integrity(struct super_block *sb, struct pmfs_super_block *super)
{
	...
	while (ino_next != 0) {
		...
		if (inode->i_nlink) {
			...
			pmfs_set_blocksize_hint(
				sb, pi, le64_to_cpu(li->i_truncatesize));
...
static int pmfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	pmfs_set_blocksize(sb, PMFS_DEF_BLOCK_SIZE_4K);
...
setup_sb:
   ...
   * truncate list has been processed */
	...
	retval = pmfs_init_ring_buffers(sbi->sockets);

	if (retval)
		goto out;

	retval = pmfs_init_agents(sbi->cpus, sbi->sockets);
...
#ifdef CONFIG_PMFS_TEST
	if (first_pmfs_super == sbi->start_virt_addr)
...
{
	return (pmfs_transaction_t *)kmem_cache_alloc(pmfs_transaction_cachep,
						      GFP_NOFS);
}
...
static struct inode *pmfs_alloc_inode(struct super_block *sb)
{
	...
	pmfs_init_range_lock(get_rangelock_info(vi));
...
static void pmfs_i_callback(struct rcu_head *head)
{
	...
	struct pmfs_inode_info *pi = PMFS_I(inode);

	range_lock_free(get_rangelock_info(pi));
	pmfs_pinode_rwsem_free(pi);

	kmem_cache_free(pmfs_inode_cachep, pi);
}
...
{
	pmfs_range_node_cachep = kmem_cache_create(
		"pmfs_range_node_cache", sizeof(struct pmfs_range_node), 0,
...
{
	pmfs_inode_cachep = kmem_cache_create(
		"pmfs_inode_cache", sizeof(struct pmfs_inode_info), 0,
...
{
	pmfs_transaction_cachep = kmem_cache_create(
		"pmfs_journal_transaction", sizeof(pmfs_transaction_t), 0,
		(SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD), NULL);
...
static int __init init_rangelock_cache(void)
{
#ifdef PMFS_ENABLE_RANGE_LOCK_KMEM_CACHE
	pmfs_rangelock_cachep = kmem_cache_create("pmfs_rangelock_cache", sizeof(struct sg_entry) * SEGMENT_INIT_COUNT, 0, (SLAB_RECLAIM_ACCOUNT |  SLAB_MEM_SPREAD), NULL);

	if(pmfs_rangelock_cachep == NULL)
	{
		pmfs_dbg("PMFS: failed to init rangelock cache\n");
		return -ENOMEM;
	}	
#endif
	return 0;
...
static void destroy_rangelock_cache(void)
{
#ifdef PMFS_ENABLE_RANGE_LOCK_KMEM_CACHE
	kmem_cache_destroy(pmfs_rangelock_cachep);
#endif
}
...
static struct dentry *pmfs_mount(struct file_system_type *fs_type, int flags,
				 const char *dev_name, void *data)
{
	return mount_nodev(fs_type, flags, data, pmfs_fill_super);
}

static void pmfs_kill_sb(struct super_block *sb)
{
...
static struct file_system_type pmfs_fs_type = {
	...
	.name = "odinfs",
	.mount = pmfs_mount,
	.kill_sb = pmfs_kill_sb,
};

static struct inode *pmfs_nfs_get_inode(struct super_block *sb, u64 ino,
					u32 generation)
{
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	struct inode *inode;
...
static struct dentry *pmfs_fh_to_dentry(struct super_block *sb, struct fid *fid,
...
static struct dentry *pmfs_fh_to_parent(struct super_block *sb, struct fid *fid,
					int fh_len, int fh_type)
{
