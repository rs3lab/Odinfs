...
unsigned int blk_type_to_shift_pmfs[PMFS_BLOCK_TYPE_MAX] = { 12, 15, 21, 30 };
uint32_t blk_type_to_size_pmfs[PMFS_BLOCK_TYPE_MAX] = { 0x1000, 0x8000,
							0x200000, 0x40000000 };
...
u64 pmfs_find_data_block(struct inode *inode, unsigned long file_blocknr)
{
	...
	pmfs_dbg1("find_data_block %lx, %x %llx blk_p %p blk_shift %x"
		  ...
		  pmfs_get_virt_addr_from_offset(sb, bp), blk_shift,
		  blk_offset);
...
static int recursive_find_region(struct super_block *sb, __le64 block,
				 ...
				 unsigned long last_blocknr, int *data_found,
				 int *hole_found, int hole)
{
	unsigned int meta_bits = META_BLK_SHIFT;
	__le64 *node;
	...
	node = pmfs_get_virt_addr_from_offset(sb, le64_to_cpu(block));
	...
	for (i = first_index; i <= last_index; i++) {
		if (height == 1 || node[i] == 0) {
			...
			first_blk = (i == first_index) ?
						  (first_blocknr &
					     ((1 << node_bits) - 1)) :
						  0;

			last_blk = (i == last_index) ?
						 (last_blocknr &
					    ((1 << node_bits) - 1)) :
						 (1 << node_bits) - 1;
...
unsigned long pmfs_find_region(struct inode *inode, loff_t *offset, int hole)
{
	...
	blocks = recursive_find_region(inode->i_sb, pi->root, pi->height,
				       first_blocknr, last_blocknr, &data_found,
				       &hole_found, hole);
	...
	if (height == 1) {
		for (i = first_index; i <= last_index; i++) {
			...
			blocknr = pmfs_get_blocknr(sb, le64_to_cpu(node[i]),
						   btype);
			pmfs_free_data_blocks(sb, blocknr,
					      pmfs_get_numblocks(btype));
			freed++;
		}
	} else {
		for (i = first_index; i <= last_index; i++) {
			...
			if (mpty) {
				...
				pmfs_free_index_blocks(
					sb, blocknr,
					pmfs_get_numblocks(PMFS_BLOCK_TYPE_4K));
			} else {
				if (i == first_index)
					start++;
...
unsigned int pmfs_free_inode_subtree(struct super_block *sb, __le64 root,
				     u32 height, u32 btype,
				     unsigned long last_blocknr)
{
	unsigned long first_blocknr;
	unsigned int freed;
	bool mpty;
	PMFS_DEFINE_TIMING_VAR(free_time);
	...
	if (height == 0) {
		first_blocknr = pmfs_get_blocknr(sb, le64_to_cpu(root), btype);
		pmfs_free_data_blocks(sb, first_blocknr,
				      pmfs_get_numblocks(btype));
		freed = 1;
	} else {
		first_blocknr = 0;
		...
		first_blocknr = pmfs_get_blocknr(sb, le64_to_cpu(root),
						 PMFS_BLOCK_TYPE_4K);
		pmfs_free_index_blocks(sb, first_blocknr,
				       pmfs_get_numblocks(PMFS_BLOCK_TYPE_4K));
	}
				       ...
				       unsigned long newsize, __le64 newroot)
{
	...
	while (height > new_height) {
		...
		root = pmfs_get_virt_addr_from_offset(sb, le64_to_cpu(newroot));
		blocknr = pmfs_get_blocknr(sb, le64_to_cpu(newroot),
					   PMFS_BLOCK_TYPE_4K);
		newroot = root[0];
		pmfs_free_index_blocks(sb, blocknr,
				       pmfs_get_numblocks(PMFS_BLOCK_TYPE_4K));
		height--;
	}
				   ...
				   loff_t end)
{
	...
	if (pi->i_flags & cpu_to_le32(PMFS_EOFBLOCKS_FL)) {
		...
		last_blocknr =
			pmfs_sparse_last_blocknr(pi->height, last_blocknr);
	}
	...
	if (pi->height == 0) {
		first_blocknr =
			pmfs_get_blocknr(sb, le64_to_cpu(root), pi->i_blk_type);
		pmfs_free_data_blocks(sb, first_blocknr,
				      pmfs_get_numblocks(pi->i_blk_type));
		root = 0;
		freed = 1;
	} else {
		freed = recursive_truncate_blocks(sb, root, pi->height,
						  pi->i_blk_type, first_blocknr,
						  last_blocknr, &mpty);
		if (mpty) {
			first_blocknr = pmfs_get_blocknr(sb, le64_to_cpu(root),
							 PMFS_BLOCK_TYPE_4K);
	...
	if (pmfs_is_mounting(sb))
		...
		inode->i_blocks -=
			(freed * (1 << (data_bits - sb->s_blocksize_bits)));
				      ...
				      struct pmfs_inode *pi, u32 new_height)
{
	...
	while (height < new_height) {
		...
		errval = pmfs_new_index_blocks(sb, pi, &blocknr, 1);
		...
		root = pmfs_get_virt_addr_from_offset(sb, blocknr);
...
static int recursive_alloc_blocks(pmfs_transaction_t *trans,
				  ...
				  unsigned long first_blocknr,
				  unsigned long last_blocknr, bool new_node,
				  bool zero)
{
	int i, errval;
	...
	for (i = first_index; i <= last_index; i++) {
         ...
         * modifying */
				...
				pmfs_memunlock_block(sb, node);
				node[i] = cpu_to_le64(pmfs_get_block_off(
					sb, blocknr, pi->i_blk_type));
				pmfs_memlock_block(sb, node);
			}
		} else {
			if (node[i] == 0) {
				/* allocate the meta block */
				errval = pmfs_new_index_blocks(sb, pi, &blocknr,
							       1);
				if (errval) {
			...
			unsigned int num, bool zero)
{
	...
	PMFS_DEFINE_TIMING_VAR(alloc_time);
	...
	pmfs_dbg_verbose("alloc_blocks height %d file_blocknr %lx num %x, "
			 ...
			 pi->height, file_blocknr, num, first_blocknr,
			 last_blocknr);
	...
	if (!pi->root) {
		if (height == 0) {
			__le64 root;
			errval = pmfs_new_data_blocks(sb, pi, &blocknr, zero);
			if (errval) {
...
struct pmfs_inode *pmfs_get_inode(struct super_block *sb, u64 ino)
{
	...
	data_bits = blk_type_to_shift_pmfs[pmfs_inode_table->i_blk_type];
	...
	for (i = 0; i < superpage_count; i++) {
		...
		curr_addr = (unsigned long) pmfs_get_virt_addr_from_offset(sb, curr);
		/* Next page pointer in the last 8 bytes of the superpage */
		curr_addr += blk_type_to_size_pmfs[pmfs_inode_table->i_blk_type] - 8;
		curr = *(u64 *)(curr_addr);

		if (curr == 0) {
			/* Make them at the first socket for now */
			allocated = pmfs_new_blocks(sb, &blocknr, 1, 1, cpuid, 0);
	...
	curr = (u64)pmfs_get_virt_addr_from_offset(sb, curr);
...
static inline int pmfs_insert_inodetree(struct pmfs_sb_info *sbi,
...
int pmfs_init_inode_inuse_list(struct super_block *sb)
{
	...
	pmfs_dbg_verbose("num_used_inodes = %u\n", sbi->s_inodes_used_count);
...
static int pmfs_alloc_inode_table(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		/* Make them at the first socket for now */
		allocated = pmfs_new_blocks(sb, &blocknr, 1, 1, i, 0);
...
int pmfs_init_inode_table(struct super_block *sb)
{
	...
	pi->nsocket = pmfs_get_init_nsocket(sb, pi);
...
static int pmfs_free_inode(struct inode *inode)
{
	...
	PMFS_DEFINE_TIMING_VAR(free_inode_time);
	...
	struct pmfs_inode_info_header *sih)
{
	...
	while (pos < inode->i_size) {
		...
		u64 bp = 0;
		bp = pmfs_find_data_block(inode, blk);
		blk_base = pmfs_get_virt_addr_from_offset(sb, bp);
		if (!blk_base) {
			pmfs_dbg("directory %lu contains a hole at offset %ld\n",
				inode->i_ino, pos);
...
static void pmfs_init_header(struct pmfs_inode_info_header *sih, u16 i_mode)
{
	sih->ino = 0;
	sih->rb_tree = RB_ROOT;
	sih->i_mode = i_mode;
...
static int pmfs_rebuild_inode(struct super_block *sb, struct inode *inode,
		struct pmfs_inode *pi)
{
	struct pmfs_inode_info *si = PMFS_I(inode);
	struct pmfs_inode_info_header *sih = &si->header;

	pmfs_init_header(sih, __le16_to_cpu(pi->i_mode));
	sih->ino = inode->i_ino;
		...
		umode_t mode, const struct qstr *qstr)
{
	...
	si = PMFS_I(inode);
	sih = &si->header;
	pmfs_init_header(sih, inode->i_mode);
	sih->ino = ino;
...
static void pmfs_block_truncate_page(struct inode *inode, loff_t newsize)
{
	...
	bp = pmfs_get_virt_addr_from_offset(sb, blockoff);
	if (!bp)
		return;
...
void pmfs_truncate_del(struct inode *inode)
{
   ...
   * inode from the truncate list */
	...
	if (prev == &sbi->s_truncate) {
		...
		struct inode *i_prv =
			&list_entry(prev, struct pmfs_inode_info, i_truncated)
				 ->vfs_inode;
...
void pmfs_truncate_add(struct inode *inode, u64 truncate_size)
{
	...
	pmfs_memlock_range(sb, head, sizeof(*head));
	pmfs_flush_buffer(&head->i_next_truncate, sizeof(head->i_next_truncate),
			  false);
...
void pmfs_setsize(struct inode *inode, loff_t newsize)
{
    ...
    /* It looks like pmfs does not need to hold locks here, maybe the lock
     * is held in the vfs? Anyway, drop the lock here for now.
     */

    /* pcpu_rwsem_down_write(pmfs_get_rwlock(inode)); */
    ...
    /* pcpu_rwsem_up_write(pmfs_get_rwlock(inode)); */
}
...
int pmfs_notify_change(struct user_namespace *mnt_userns, struct dentry *dentry,
		       struct iattr *attr)
{
