 ...
 */
#define SOLROS_RING_BUFFER_NON_BLOCKING    0x0
#define SOLROS_RING_BUFFER_BLOCKING        0x1
...
struct solros_ring_buffer_req_t {
	...
	size_t       size;   /* [in] size */
	volatile struct solros_ring_buffer_req_t *__next; /* [internal] next waiter */
...
#define SOLROS_RING_BUFFER_REQ_BLOCKING       0x0001
#define SOLROS_RING_BUFFER_REQ_NON_BLOCKING   0x0002
#define SOLROS_RING_BUFFER_REQ_COMBINER       0x4000
#define SOLROS_RING_BUFFER_REQ_DONE           0x8000

#define solros_ring_buffer_get_req_init(__req, __md) do {		\
		(__req)->__next = NULL;				\
		(__req)->rc = -EINPROGRESS;			\
		(__req)->flag   = SOLROS_RING_BUFFER_REQ_##__md;	\
	} while(0)
#define solros_ring_buffer_put_req_init(__req, __md, __sz) do {	\
		solros_ring_buffer_get_req_init(__req, __md);		\
		(__req)->size = __sz;		                \
	} while(0)
#define solros_ring_buffer_poll_request(__req) (		  \
		(__req)->flag & SOLROS_RING_BUFFER_REQ_DONE)
 ...
 */
typedef void (*solros_ring_buffer_reap_cb_t)(void*, void *);
#ifdef __cplusplus
}
#endif
