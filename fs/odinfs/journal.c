				      ...
				      pmfs_logentry_t *le)
{
	...
	if (le->size > 0) {
		data = pmfs_get_virt_addr_from_offset(
...
static int pmfs_journal_cleaner_run(struct super_block *sb)
{
	...
	sbi->log_cleaner_thread =
		kthread_run(pmfs_log_cleaner, sb, "pmfs_log_cleaner_0x%llx",
			    sbi->phys_addr);
...
int pmfs_journal_soft_init(struct super_block *sb)
{
	...
	sbi->journal_base_addr =
		pmfs_get_virt_addr_from_offset(sb, le64_to_cpu(journal->base));
...
int pmfs_journal_hard_init(struct super_block *sb, uint64_t base, uint32_t size)
{
	...
	sbi->journal_base_addr = pmfs_get_virt_addr_from_offset(sb, base);
					 ...
					 int max_log_entries)
{
	...
	uint64_t base;
	int retry = 0;
	PMFS_DEFINE_TIMING_VAR(log_time);
#if 0
...
again:
	...
	avail_size =
		(tail >= head) ? (sbi->jsize - (tail - head)) : (head - tail);
   ...
   * that we don't have any wraparound within a transaction */
	...
	pmfs_dbg_trans("new transaction tid %d nle %d avl sz %x sa %llx\n",
		       ...
		       base);
	trans->start_addr = pmfs_get_virt_addr_from_offset(sb, base);
		      ...
		      void *addr, uint16_t size, u8 type)
{
	...
	uint8_t le_size;
	PMFS_DEFINE_TIMING_VAR(add_log_time);
...
int pmfs_commit_transaction(struct super_block *sb, pmfs_transaction_t *trans)
{
	PMFS_DEFINE_TIMING_VAR(commit_time);
