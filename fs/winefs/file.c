...
static inline int winefs_can_set_blocksize_hint(struct winefs_inode *pi,
					       loff_t new_size)
{
		...
		loff_t new_size)
{
	...
	if (!winefs_can_set_blocksize_hint(pi, new_size))
		return 0;

	if (new_size >= 0x40000000) {   /* 1G */
		block_type = WINEFS_BLOCK_TYPE_1G;
		goto hint_set;
	}

	if (new_size >= 0x200000) {     /* 2M */
		block_type = WINEFS_BLOCK_TYPE_2M;
		goto hint_set;
	}

	/* defaulting to 4K */
	block_type = WINEFS_BLOCK_TYPE_4K;

hint_set:
	winefs_dbg_verbose(
		"Hint: new_size 0x%llx, i_size 0x%llx, root 0x%llx\n",
		new_size, pi->i_size, le64_to_cpu(pi->root));
	winefs_dbg_verbose("Setting the hint to 0x%x\n", block_type);
...
static long winefs_fallocate(struct file *file, int mode, loff_t offset,
			    loff_t len)
{
	...
	trans = winefs_new_transaction(sb, MAX_INODE_LENTRIES +
				     MAX_METABLOCK_LENTRIES, winefs_get_cpuid(sb));
	if (IS_ERR(trans)) {
		ret = PTR_ERR(trans);
		goto out;
	}
	...
	//winefs_set_blocksize_hint(sb, pi, new_size);
	...
	ret = winefs_alloc_blocks_weak(trans, inode, blocknr,
...
static loff_t winefs_llseek(struct file *file, loff_t offset, int origin)
{
	struct inode *inode = file->f_path.dentry->d_inode;
	int retval;
	...
	switch (origin) {
	case SEEK_DATA:
		retval = winefs_find_region(inode, &offset, 0);
		if (retval) {
			inode_unlock(inode);
			return retval;
		}
		break;
	case SEEK_HOLE:
		retval = winefs_find_region(inode, &offset, 1);
		if (retval) {
			inode_unlock(inode);
			return retval;
		}
 ...
 * TODO: Check if we can avoid calling winefs_flush_buffer() for fsync. We use
 * movnti to write data to files, so we may want to avoid doing unnecessary
 * winefs_flush_buffer() on fsync() */
int winefs_fsync(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	WINEFS_START_TIMING(fsync_t, fsync_time);
	...
	if (!isize || (start >= end))
	{
		winefs_dbg_verbose("[%s:%d] : (ERR) isize(%llx), start(%llx),"
			" end(%llx)\n", __func__, __LINE__, isize, start, end);
		WINEFS_END_TIMING(fsync_t, fsync_time);
		return -ENODATA;
	}
	...
	do {
		...
		winefs_find_data_blocks(inode, (sector_t)pgoff, &block, 1);
		if (block) {
			xip_mem = winefs_get_block(inode->i_sb, block);
			/* flush the range */
			atomic64_inc(&winefs_fsync_pages);
			winefs_flush_buffer(xip_mem + offset, nr_flush_bytes, 0);
		} else {
			/* sparse files could have such holes */
			winefs_dbg_verbose("[%s:%d] : start(%llx), end(%llx),"
...
static int winefs_flush(struct file *file, fl_owner_t id)
{
	int ret = 0;
...
winefs_get_unmapped_area(struct file *file, unsigned long addr,
			...
			unsigned long flags)
{
	...
	struct winefs_inode *pi = winefs_get_inode(inode->i_sb, inode->i_ino);
	struct vm_unmapped_area_info info;

	if (len > TASK_SIZE)
		return -ENOMEM;

	if (pi->i_blk_type == WINEFS_BLOCK_TYPE_1G)
		align_size = PUD_SIZE;
	else if (pi->i_blk_type == WINEFS_BLOCK_TYPE_2M)
		align_size = PMD_SIZE;
	else
...
const struct file_operations winefs_xip_file_operations = {
	.llseek			= winefs_llseek,
	.read			= winefs_xip_file_read,
	.write			= winefs_xip_file_write,
//	.aio_read		= xip_file_aio_read,
//	.aio_write		= xip_file_aio_write,
//	.read_iter		= generic_file_read_iter,
//	.write_iter		= generic_file_write_iter,
	.mmap			= winefs_xip_file_mmap,
 	.get_unmapped_area      = thp_get_unmapped_area,
	.open			= generic_file_open,
	.fsync			= winefs_fsync,
	.flush			= winefs_flush,
//	.get_unmapped_area	= winefs_get_unmapped_area,
	.unlocked_ioctl		= winefs_ioctl,
	.fallocate		= winefs_fallocate,
#ifdef CONFIG_COMPAT
	.compat_ioctl		= winefs_compat_ioctl,
#endif
};

const struct inode_operations winefs_file_inode_operations = {
	.setattr	= winefs_notify_change,
	.getattr	= winefs_getattr,
