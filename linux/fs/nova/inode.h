	...
	struct inode *inode)
{
	...
	if (memcpy(&fake_pi, addr, sizeof(struct nova_inode)) == NULL)
		return NULL;
...
extern int nova_notify_change(struct user_namespace *mnt_userns, struct dentry *dentry,
			      struct iattr *attr);
extern int nova_getattr(struct user_namespace *mnt_userns, const struct path *path,
