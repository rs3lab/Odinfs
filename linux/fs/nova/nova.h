	...
	struct nova_inode *pi, struct nova_inode_info_header *sih)
{
	...
	if (memcpy(&fake_pi, pi, sizeof(struct nova_inode)) == NULL)
		return -1;
...
static inline u64 next_log_page(struct super_block *sb, u64 curr)
{
	...
	if (memcpy(&next, &curr_page->page_tail.next_page,
		    sizeof(u64)) == NULL)
		return 1;
...
static inline u64 alter_log_page(struct super_block *sb, u64 curr)
{
	...
	curr = BLOCK_OFF(curr);
	curr_page = (struct nova_inode_log_page *)nova_get_block(sb, curr);
	if (memcpy(&next, &curr_page->page_tail.alter_page, sizeof(u64)) == NULL)
		return 1;
...
static inline bool goto_next_page(struct super_block *sb, u64 curr_p)
{
	...
	if (memcpy(&type, addr, sizeof(u8)) == NULL)
		rc = -1;
