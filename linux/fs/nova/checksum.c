	...
	u32 *entry_csum, size_t *entry_size, void *entry_copy)
{
	...
	if (memcpy(&type, entry, sizeof(u8)) == NULL)
		return -1;

	switch (type) {
	case DIR_LOG:
		dentry = DENTRY(entry_copy);
		if (memcpy(dentry, entry, NOVA_DENTRY_HEADER_LEN) == NULL)
			ret = -1;
	...
	case FILE_WRITE:
		*entry_size = sizeof(struct nova_file_write_entry);
		if (memcpy(entry_copy, entry, *entry_size) == NULL)
			break;
...
static int nova_repair_entry_pr(struct super_block *sb, void *entry)
{
	...
	if (memcpy(entry_pr, alter_pr, POISON_RADIUS) == NULL)
		ret = -1;
	...
	struct nova_inode *bad_pi, struct nova_inode *good_pi)
{
	...
	if (memcpy(bad_pr, good_pr, POISON_RADIUS) == NULL)
		ret = -1;
	...
	u64 alter_pi_addr, struct nova_inode *pic, int check_replica)
{
	...
	if (memcpy(pic, pi, sizeof(struct nova_inode)) == NULL)
		ret = -1;
	...
	if (memcpy(alter_pic, alter_pi, sizeof(struct nova_inode)) == NULL)
		ret = -1;
	if (ret < 0) { /* media error */
		if (inode_bad)
			goto fail;
	...
	size_t offset, size_t bytes)
{
	...
	for (strp = 0; strp < strps; strp++) {
		...
		if (memcpy(strip, strp_ptr, strp_size) == NULL)
			error = -1;
		if (error < 0) {
...
int nova_update_truncated_block_csum(struct super_block *sb,
	...
	if (strp_offset > 0) {
		...
		if (memcpy(tail_strp, strp_addr, strp_offset) == NULL)
			return -EIO;
