...
#define INODES_PER_BLOCK(bt) (1 << (blk_type_to_shift_pmfs[bt] - PMFS_INODE_BITS))
...
extern unsigned long long Timingstats_pmfs[TIMING_NUM];
extern u64 Countstats_pmfs[TIMING_NUM];
...
#define PMFS_START_TIMING(name, start) \
	{if (measure_timing_pmfs) ktime_get_ts64(&start);}

#define PMFS_END_TIMING(name, start) \
	{if (measure_timing_pmfs) { \
		timing_t end; \
		ktime_get_ts64(&end); \
		Timingstats_pmfs[name] += \
			(end.tv_sec - start.tv_sec) * 1000000000 + \
			(end.tv_nsec - start.tv_nsec); \
	} \
	Countstats_pmfs[name]++; \
	}
