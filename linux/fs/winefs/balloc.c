...
// manage.
static void winefs_init_free_list(struct super_block *sb,
	struct free_list *free_list, int index)
{
	...
	if (sbi->num_numa_nodes == 2) {
		if (index < second_node_cpuid && free_list->block_start >= sbi->block_start[1]) {
			winefs_dbg("%s: Wrong NUMA setting: CPU id = %d, free_list->block_start = %lu",
...
void winefs_init_blockmap(struct super_block *sb,
			...
			int recovery)
{
	...
	winefs_dbg_verbose("%s: sbi->head_reserved_blocks = %lu\n", __func__,
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		winefs_init_free_list(sb, free_list, i);
		...
		if (recovery == 0) {
			...
			if (unaligned_end != unaligned_start) {
				...
				ret = winefs_insert_blocktree(unaligned_tree, blknode);
				if (ret) {
			...
			winefs_dbg_verbose("%s: number of huge pages in list %d = %lu\n",
			...
			for (j = aligned_start; j < aligned_end; j += PAGES_PER_2MB) {
				...
				ret = winefs_insert_blocktree(huge_aligned_tree, blknode);
				if (ret) {
					winefs_err(sb, "%s failed\n", __func__);
					winefs_free_blocknode(sb, blknode);
					return;
...
static inline int winefs_rbtree_compare_rangenode(struct winefs_range_node *curr,
...
int winefs_find_range_node(struct rb_root *tree, unsigned long key,
			 enum node_type type, struct winefs_range_node **ret_node)
{
	struct winefs_range_node *curr = NULL;
	struct rb_node *temp;
	int compVal;
	int ret = 0;
	...
	while (temp) {
		...
		compVal = winefs_rbtree_compare_rangenode(curr, key, type);
...
int winefs_insert_range_node(struct rb_root *tree,
			   struct winefs_range_node *new_node, enum node_type type)
{
	...
	while (*temp) {
		curr = container_of(*temp, struct winefs_range_node, node);
		compVal = winefs_rbtree_compare_rangenode(curr,
		...
		if (compVal == -1) {
			...
			winefs_dbg("%s: entry %lu - %lu already exists: "
				"%lu - %lu\n",
			  ...
			  struct winefs_range_node *new_node)
{
	...
	ret = winefs_insert_range_node(tree, new_node, NODE_BLOCK);
	if (ret)
...
struct winefs_range_node *winefs_alloc_blocknode_atomic(struct super_block *sb)
{
	return winefs_alloc_range_node_atomic(sb);
}
 ...
 * 512
 */
bool winefs_alloc_superpage(struct super_block *sb,
	struct free_list *free_list, unsigned long num_blocks,
	unsigned long *new_blocknr)
{
	struct rb_root *tree;
	struct rb_node *temp;
	struct winefs_range_node *curr, *node;
	bool found = 0;
	...
	if (num_blocks != PAGES_PER_2MB) {
		winefs_dbg("%s: wrong number of blocks. Expected = 512. Requested = %lu\n",
			 __func__, num_blocks);
		dump_stack();
		return found;
	}
	...
	if (temp) {
		...
		*new_blocknr = curr->range_low;
		...
		temp = rb_next(temp);
		if (temp)
			node = container_of(temp, struct winefs_range_node, node);
		free_list->first_node_huge_aligned = node;

		/* release curr after updating {first, last}_node */
		rb_erase(&curr->node, tree);
		winefs_free_blocknode(sb, curr);
		free_list->num_blocknode_huge_aligned--;
		found = 1;
	}

	winefs_dbg_verbose("%s: blocknr = %lu. num_blocks = %lu\n",
...
static long winefs_alloc_blocks_in_free_list(struct super_block *sb,
	struct free_list *free_list, unsigned short btype,
	unsigned long num_blocks,
	unsigned long *new_blocknr)
{
	struct rb_root *tree, *huge_tree;
	struct winefs_range_node *curr, *node, *next = NULL, *prev = NULL, *blknode;
	...
	if ((!free_list->first_node_unaligned &&
	    ...
	    free_list->num_free_blocks == 0) {
		winefs_dbg("%s: Can't alloc. free_list->first_node_unaligned=0x%p "
	...
	winefs_dbg_verbose("%s: Got allocation req for num_blocks = %lu\n",
			 __func__, num_blocks);
	...
	if (IS_DATABLOCKS_2MB_ALIGNED(num_blocks)) {
		found_hugeblock = winefs_alloc_superpage(sb, free_list,
	...
	if (!temp) {
		...
		blknode = winefs_alloc_blocknode(sb);
		if (blknode == NULL) {
			winefs_dbg("%s: alloc blocknode failed\n", __func__);
			return -ENOMEM;
		}
		...
		if (tree == NULL) {
			winefs_dbg("%s: unaligned tree does not exist\n", __func__);
			WINEFS_ASSERT(0);
		}

		winefs_insert_blocktree(tree, blknode);
		free_list->first_node_unaligned = blknode;

		winefs_dbg_verbose("%s: breaking an aligned free space from hugepage rb tree "
		...
		if (next_node)
			next = container_of(next_node, struct winefs_range_node, node);
	...
	while (temp) {
		...
		winefs_dbg_verbose("%s: curr->range_low = %lu. "
...
next:
	...
	if (free_list->num_free_blocks < num_blocks) {
		winefs_dbg("%s: free list %d has %lu free blocks, "
...
success:
	if ((found == 1) || (found_hugeblock == 1))
		...
		winefs_dbg("%s: Can't alloc.  found = %d", __func__, found);
		return -ENOSPC;
	}
	...
	struct winefs_range_node **next)
{
	...
	ret = winefs_find_range_node(tree, range_low, NODE_BLOCK, &ret_node);
	if (ret) {
		winefs_dbg("%s ERROR: %lu - %lu already in free list\n",
	...
	if (!ret_node) {
		...
		*prev = ret_node;
		...
		if (tmp) {
			*next = container_of(tmp, struct winefs_range_node, node);
			check_next = 1;
		} else {
			*next = NULL;
		}
		...
		*next = ret_node;
		...
		if (tmp) {
			*prev = container_of(tmp, struct winefs_range_node, node);
			check_prev = 1;
		} else {
			*prev = NULL;
		}
	} else {
		winefs_dbg("%s ERROR: %lu - %lu overlaps with existing "
			 "node %lu - %lu\n",
			       ...
			       struct free_list *free_list)
{
	...
	if (huge_tree == NULL) {
		winefs_dbg("%s: huge_tree does not exist\n", __func__);
		WINEFS_ASSERT(0);
	}

	ret = winefs_insert_blocktree(huge_tree, node);
	if (ret) {
		goto out;
	}
...
static int check_and_insert_huge_aligned(struct super_block *sb,
					 ...
					 struct winefs_range_node *new_node,
					 struct winefs_range_node *old_node,
					 struct free_list *free_list,
					 int *new_node_used)
{
	...
	int ret = 0;
	int idx = 0;
	struct winefs_range_node *node, *next = NULL;
	...
	if (unaligned_block_low == block_high + 1) {
		if (new_unaligned_node_needed == 1) {
			...
			if (node == NULL || unaligned_tree == NULL) {
				winefs_dbg("%s: node or unaligned tree does not exist\n", __func__);
				WINEFS_ASSERT(0);
			}

			ret = winefs_insert_blocktree(unaligned_tree, node);
			if (ret) {
				goto out;
			}
			free_list->num_blocknode_unaligned++;
	...
	if (unaligned_block_low == 0 && unaligned_block_high == 0) {
		...
		free_list->num_blocknode_unaligned--;
		winefs_free_blocknode(sb, old_node);
	}
	...
	for (idx = 0; idx < num_hugepages; idx++) {
		if (!(*new_node_used)) {
			...
			*new_node_used = 1;
			...
			node = winefs_alloc_blocknode(sb);
			if (node == NULL) {
				ret = -ENOMEM;
				goto out;
			}
...
int winefs_free_blocks(struct super_block *sb, unsigned long blocknr,
	int num, unsigned short btype)
{
	...
	struct winefs_range_node *prev = NULL;
	struct winefs_range_node *next = NULL;
	...
	num_blocks = winefs_get_numblocks(btype) * num;

	while (num_blocks) {

		/* Pre-allocate blocknode */
		curr_node = winefs_alloc_blocknode(sb);
		if (curr_node == NULL) {
		...
		winefs_dbg_verbose("Free: %lu - %lu\n", block_low, block_high);

		if (blocknr < free_list->block_start ||
		    blocknr + num > free_list->block_end + 1) {
			winefs_err(sb, "free blocks %lu to %lu, free list %d, "
				 "start %lu, end %lu\n",
		...
		ret = winefs_find_free_slot(tree, block_low,
		...
		if (tree == NULL) {
			winefs_dbg("%s: tree does not exist\n", __func__);
			WINEFS_ASSERT(0);
		}

		ret = winefs_insert_blocktree(tree, curr_node);
		if (ret) {
			new_node_used = 0;
			goto out;
		}
	...
	block_found:
		...
		if (new_node_used == 0)
			winefs_free_blocknode(sb, curr_node);
...
static int not_enough_holes(struct free_list *free_list,
			    unsigned long num_blocks)
{
	struct winefs_range_node *first_unaligned = free_list->first_node_unaligned;
	struct winefs_range_node *first_huge_aligned = free_list->first_node_huge_aligned;
	...
	if (num_hole_blocks < num_blocks ||
	    !first_unaligned) {
		winefs_dbg_verbose("%s: num_free_blocks=%ld; num_blocks=%ld; "
...
static int winefs_get_candidate_free_list(struct super_block *sb, int num_req_blocks)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct free_list *free_list;
	int cpuid = 0;
	...
	if (sbi->num_numa_nodes == 2) {
		numa_node = winefs_get_numa_node(sb, winefs_get_cpuid(sb));
		numa_cpus = sbi->numa_cpus;
	again:
		for (i = 0; i < numa_cpus[numa_node].num_cpus; i++) {
			free_list = winefs_get_free_list(sb, numa_cpus[numa_node].cpus[i]);
...
static int winefs_get_candidate_hole_free_list(struct super_block *sb, unsigned long num_blocks)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct free_list *free_list;
	int cpuid = 0;
	...
	if (sbi->num_numa_nodes == 1) {
		if (sbi->hole_free_list == NULL) {
			...
			sbi->hole_free_list = winefs_get_free_list(sb, cpuid);
			...
			for (i = 0; i < 3; i++) {
				...
				free_list = winefs_get_free_list(sb, candidates[i]);
...
int winefs_new_blocks(struct super_block *sb, unsigned long *blocknr,
		    unsigned int num, unsigned short btype, int zero, int cpuid)
{
	...
	num_blocks = num * winefs_get_numblocks(btype);
	if (num_blocks == 0) {
		winefs_dbg_verbose("%s: num_blocks == 0", __func__);
		return -EINVAL;
	}
	...
	if (!IS_DATABLOCKS_2MB_ALIGNED(num_blocks)) {
		...
		if (not_enough_holes(free_list, num_blocks)) {
			spin_unlock(&free_list->s_lock);
			cpuid = winefs_get_candidate_hole_free_list(sb, num_blocks);
		} else {
			goto alloc;
		}
	}
 ...
 retry:
	...
	if (not_enough_blocks(free_list, num_blocks)) {
		winefs_dbg_verbose("%s: cpu %d, free_blocks %lu, required %lu, "
			  "blocknode %lu\n",
		...
		cpuid = winefs_get_candidate_free_list(sb, num_blocks);
		retried++;
		goto retry;
	}
alloc:
	ret_blocks = winefs_alloc_blocks_in_free_list(sb, free_list, btype,
					num_blocks, &new_blocknr);

	spin_unlock(&free_list->s_lock);

	if (ret_blocks <= 0 || new_blocknr == 0) {
		winefs_dbg("%s: not able to allocate %d blocks. "
	...
	if (zero) {
		bp = winefs_get_block(sb, winefs_get_block_off(sb,
							   new_blocknr, btype));
		winefs_memunlock_range(sb, bp, PAGE_SIZE * ret_blocks);
		memset_nt(bp, 0, PAGE_SIZE * ret_blocks);
		winefs_memlock_range(sb, bp, PAGE_SIZE * ret_blocks);
	}
	*blocknr = new_blocknr;

	winefs_dbg_verbose("Alloc %lu NVMM blocks 0x%lx\n", ret_blocks, *blocknr);
	return ret_blocks / winefs_get_numblocks(btype);
}

unsigned int winefs_get_free_numa_node(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct free_list *free_list;
...
unsigned long winefs_count_free_blocks(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct free_list *free_list;
