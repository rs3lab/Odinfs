/*
 * WINEFS journaling facility. This file contains code to log changes to pmfs
...
static void dump_transaction(struct winefs_sb_info *sbi,
		winefs_transaction_t *trans)
{
	int i;
	winefs_logentry_t *le = trans->start_addr;

	for (i = 0; i < trans->num_entries; i++) {
		winefs_dbg_trans("ao %llx tid %x gid %x type %x sz %x\n",
...
static inline void winefs_undo_logentry(struct super_block *sb,
	winefs_logentry_t *le)
{
	char *data;

	if (le->size > 0) {
		data = winefs_get_block(sb, le64_to_cpu(le->addr_offset));
		/* Undo changes by flushing the log entry to pmfs */
		winefs_memunlock_range(sb, data, le->size);
		memcpy(data, le->data, le->size);
		winefs_memlock_range(sb, data, le->size);
		winefs_flush_buffer(data, le->size, false);
	}
}

/* can be called during journal recovery or transaction abort */
/* We need to Undo in the reverse order */
static void winefs_undo_transaction(struct super_block *sb,
		winefs_transaction_t *trans)
{
	winefs_logentry_t *le;
	int i;
	...
	for (i = trans->num_used - 1; i >= 0; i--, le--) {
		if (gen_id == le16_to_cpu(le->gen_id))
			winefs_undo_logentry(sb, le);
	}
}

/* can be called by either during log cleaning or during journal recovery */
static void winefs_flush_transaction(struct super_block *sb,
		winefs_transaction_t *trans)
{
	...
	for (i = 0; i < trans->num_used; i++, le++) {
		if (le->size) {
			data = winefs_get_block(sb,le64_to_cpu(le->addr_offset));
			winefs_dbg_verbose("%s: data = 0x%p. size = %d\n", __func__, data, le->size);
			if (sbi->redo_log) {
				winefs_memunlock_range(sb, data, le->size);
				memcpy(data, le->data, le->size);
				winefs_memlock_range(sb, data, le->size);
...
static inline void invalidate_gen_id(winefs_logentry_t *le)
{
	le->gen_id = 0;
	winefs_flush_buffer(le, LOGENTRY_SIZE, false);
}

/* can be called by either during log cleaning or during journal recovery */
static void winefs_invalidate_logentries(struct super_block *sb,
		winefs_transaction_t *trans)
{
	winefs_logentry_t *le = trans->start_addr;
	int i;

	winefs_memunlock_range(sb, trans->start_addr,
	...
	winefs_memlock_range(sb, trans->start_addr,
			trans->num_entries * LOGENTRY_SIZE);
}

/* can be called by either during log cleaning or during journal recovery */
static void winefs_redo_transaction(struct super_block *sb,
		winefs_transaction_t *trans, bool recover)
{
	winefs_logentry_t *le = trans->start_addr;
	int i;
	uint16_t gen_id = trans->gen_id;
...
static uint32_t winefs_recover_transaction(struct super_block *sb, uint32_t head,
		uint32_t tail, winefs_logentry_t *le)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_transaction_t trans;
	...
	if (start_found && !cmt_or_abrt_found)
		winefs_undo_transaction(sb, &trans);

	if (gen_id == MAX_GEN_ID) {
		if (!start_found)
			trans.start_addr = le;
		/* make sure the changes made by winefs_undo_transaction() are
		 * persistent before invalidating the log entries */
		if (start_found && !cmt_or_abrt_found) {
			PERSISTENT_MARK();
			PERSISTENT_BARRIER();
		}
		winefs_invalidate_logentries(sb, &trans);
	}
	return tail;
}
...
static uint32_t winefs_process_transaction(struct super_block *sb, uint32_t head,
	uint32_t tail, winefs_logentry_t *le, bool recover, int *processed)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_transaction_t trans;
	uint16_t gen_id;
	...
	*processed = 0;
	...
	if (!(le->type & LE_START)) {
		winefs_dbg("start of trans %x but LE_START not set. gen_id %d\n",
	...
	do {
		...
		if ((gen_id == le16_to_cpu(le->gen_id)) && (le->type & LE_COMMIT
			...
			if ((le->type & LE_COMMIT) && sbi->redo_log)
				winefs_redo_transaction(sb, &trans, recover);
		...
		if ((new_head == tail) || ((gen_id == le16_to_cpu(le->gen_id))
			...
			winefs_dbg_trans("no cmt tid %d sa %p nle %d tail %x"
			" gen %d\n",
...
static int winefs_clean_journal(struct super_block *sb, bool unmount, int cpu,
	int take_lock)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_journal_t *journal = winefs_get_journal(sb, cpu);
	uint32_t head;
	...
	winefs_dbg_trans("starting journal cleaning %x %x\n", head, tail);
	while (head != tail) {
		le = (winefs_logentry_t *)(sbi->journal_base_addr[cpu] + head);
		if (gen_id == le16_to_cpu(le->gen_id)) {
			/* found a valid log entry, process the transaction */
			new_head = winefs_process_transaction(sb, head, tail,
				le, false, &processed);
			total += processed;
			...
			if (gen_id == MAX_GEN_ID) {
				winefs_memunlock_range(sb, le, sizeof(*le));
				invalidate_gen_id(le);
				winefs_memlock_range(sb, le, sizeof(*le));
			}
	...
	PERSISTENT_MARK();
	PERSISTENT_BARRIER();
	winefs_memunlock_range(sb, journal, sizeof(*journal));
	journal->head = cpu_to_le32(head);
	winefs_memlock_range(sb, journal, sizeof(*journal));
	winefs_flush_buffer(&journal->head, sizeof(journal->head), true);
	if (unmount) {
		PERSISTENT_MARK();
		if (journal->head != journal->tail)
			winefs_dbg("WINEFS: umount but journal not empty %x:%x\n",
			le32_to_cpu(journal->head), le32_to_cpu(journal->tail));
		PERSISTENT_BARRIER();
	}
	winefs_dbg_trans("leaving journal cleaning %x %x\n", head, tail);
	if (take_lock)
		mutex_unlock(&sbi->journal_mutex[cpu]);
	return total;
}

static void log_cleaner_try_sleeping(struct  winefs_sb_info *sbi)
{
	DEFINE_WAIT(wait);
...
static int winefs_log_cleaner(void *arg)
{
	struct super_block *sb = (struct super_block *)arg;
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	int i;

	winefs_dbg_trans("Running log cleaner thread\n");
	for ( ; ; ) {
		...
		for (i = 0; i < sbi->cpus; i++)
			winefs_clean_journal(sb, false, i, 1);
	}

	for (i = 0; i < sbi->cpus; i++)
		winefs_clean_journal(sb, true, i, 1);

	winefs_dbg_trans("Exiting log cleaner thread\n");
	return 0;
}

static int winefs_journal_cleaner_run(struct super_block *sb)
{
	int ret = 0;
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	...
	sbi->log_cleaner_thread = kthread_run(winefs_log_cleaner, sb,
			"winefs_log_cleaner_0x%llx", sbi->phys_addr);
	if (IS_ERR(sbi->log_cleaner_thread)) {
		/* failure at boot is fatal */
		winefs_err(sb, "Failed to start pmfs log cleaner thread\n");
		ret = -1;
	}
	return ret;
}

int winefs_journal_soft_init(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_journal_t *journal = winefs_get_journal(sb, 0);
	...
	journal_meta_start = sizeof(struct winefs_super_block);
	...
	for (i = 0; i < sbi->cpus; i++) {
		journal = winefs_get_journal(sb, i);
		sbi->journal_base_addr[i] = winefs_get_block(sb,le64_to_cpu(journal->base));
		mutex_init(&sbi->journal_mutex[i]);
		sbi->redo_log = !!le16_to_cpu(journal->redo_logging);
	}

	return winefs_journal_cleaner_run(sb);
}

int winefs_journal_hard_init(struct super_block *sb, uint64_t base,
	uint32_t size)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_journal_t *journal;
	int i;
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		sbi->journal_base_addr[i] = winefs_get_block(sb, base + (size*i));
		winefs_memunlock_range(sb, sbi->journal_base_addr[i], size);
		memset_nt(sbi->journal_base_addr[i], 0, size);
		winefs_memlock_range(sb, sbi->journal_base_addr[i], size);
	}

	return winefs_journal_soft_init(sb);
}

static void wakeup_log_cleaner(struct winefs_sb_info *sbi)
{
	if (!waitqueue_active(&sbi->log_cleaner_wait))
		return;
	winefs_dbg_trans("waking up the cleaner thread\n");
	wake_up_interruptible(&sbi->log_cleaner_wait);
}

int winefs_journal_uninit(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
...
inline winefs_transaction_t *winefs_current_transaction(void)
{
	return (winefs_transaction_t *)current->journal_info;
}

static int winefs_free_logentries(struct super_block *sb, int max_log_entries, int cpu)
{
	int freed_entries = 0;

	freed_entries = winefs_clean_journal(sb, false, cpu, 0);
...
winefs_transaction_t *winefs_new_transaction(struct super_block *sb,
					 int max_log_entries, int cpu)
{
	...
	WINEFS_START_TIMING(new_trans_t, log_time);

	if (!sbi->redo_log)
		max_log_entries++;

	trans = winefs_alloc_transaction();
	if (!trans)
...
again:
	...
	if (avail_size < req_size) {
		...
		for (retry = 0; retry < 3; retry++) {
			freed_size += winefs_free_logentries(sb,
							   max_log_entries, cpu);
	...
	if (tail >= sbi->jsize) {
		...
		winefs_memlock_range(sb, journal, sizeof(*journal));
		winefs_dbg_trans("journal wrapped. tail %x gid %d cur tid %d\n",
	...
	winefs_flush_buffer(&journal->tail, sizeof(u64), false);
	mutex_unlock(&sbi->journal_mutex[cpu]);
	...
	winefs_dbg_trans("new transaction tid %d nle %d avl sz %x sa %llx\n",
		trans->transaction_id, max_log_entries, avail_size, base);
	trans->start_addr = winefs_get_block(sb, base);

	trans->parent = (winefs_transaction_t *)current->journal_info;
	current->journal_info = trans;
	WINEFS_END_TIMING(new_trans_t, log_time);
	return trans;
journal_full:
	mutex_unlock(&sbi->journal_mutex[cpu]);
	winefs_err(sb, "Journal full. base %llx sz %x head:tail %x:%x ncl %x\n",
		le64_to_cpu(journal->base), le32_to_cpu(journal->size),
		le32_to_cpu(journal->head), le32_to_cpu(journal->tail),
		max_log_entries);
	winefs_err(sb, "avail size %u, freed size %u, request size %u\n",
		avail_size, freed_size, req_size);
	winefs_free_transaction(trans);
	WINEFS_END_TIMING(new_trans_t, log_time);
	return ERR_PTR(-EAGAIN);
}

static inline void winefs_commit_logentry(struct super_block *sb,
		winefs_transaction_t *trans, winefs_logentry_t *le)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	if (sbi->redo_log) {
		/* Redo Log */
		...
		winefs_flush_transaction(sb, trans);
	} else {
		/* Undo Log */
		...
		/* Atomically make the log entry valid */
		le->gen_id = cpu_to_le16(trans->gen_id);
		winefs_flush_buffer(le, LOGENTRY_SIZE, true);
	}
}

int winefs_add_logentry(struct super_block *sb,
		winefs_transaction_t *trans, void *addr, uint16_t size, u8 type)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_logentry_t *le;
	int num_les = 0, i;
	uint64_t le_start = size ? winefs_get_addr_off(sbi, addr) : 0;
	uint8_t le_size;
	...
	WINEFS_START_TIMING(add_log_t, add_log_time);
	le = trans->start_addr + trans->num_used;
	...
	winefs_dbg_trans("add le id %d size %x, num_les %d tail %x le %p\n",
		trans->transaction_id, size, trans->num_entries,
		trans->num_used, le);

	if ((trans->num_used + num_les) > trans->num_entries) {
		winefs_err(sb, "Log Entry full. tid %lu ne %d tail %x size %x\n",
	...
	if (addr) {
		winefs_dbg_verbose("%s: trans start addr = 0x%p. le = 0x%p. le_start = 0x%llu. num_les = %d. param addr = 0x%p. size = %u. type = %u\n",
				 __func__, trans->start_addr, le, le_start, num_les, addr, size, type);
	}

	winefs_memunlock_range(sb, le, sizeof(*le) * num_les);
	for (i = 0; i < num_les; i++) {
		...
		if (i == (num_les - 1) && (type & LE_COMMIT)) {
			winefs_commit_logentry(sb, trans, le);
			winefs_memlock_range(sb, le, sizeof(*le) * num_les);
			WINEFS_END_TIMING(add_log_t, add_log_time);
			return 0;
		}
...
int winefs_commit_transaction(struct super_block *sb,
		winefs_transaction_t *trans)
{
	...
	winefs_add_logentry(sb, trans, NULL, 0, LE_COMMIT);

	WINEFS_START_TIMING(commit_trans_t, commit_time);
	winefs_dbg_trans("completing transaction for id %d\n",
		trans->transaction_id);

	current->journal_info = trans->parent;
	winefs_free_transaction(trans);
	WINEFS_END_TIMING(commit_trans_t, commit_time);
	return 0;
}

int winefs_abort_transaction(struct super_block *sb, winefs_transaction_t *trans)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);

	if (trans == NULL)
		return 0;
	winefs_dbg_trans("abort trans for tid %x sa %p numle %d tail %x gen %d\n",
		trans->transaction_id, trans->start_addr, trans->num_entries,
		trans->num_used, trans->gen_id);
	dump_transaction(sbi, trans);
	/*dump_stack();*/
	...
	winefs_add_logentry(sb, trans, NULL, 0, LE_ABORT);
	current->journal_info = trans->parent;
	winefs_free_transaction(trans);
	return 0;
}
...
{
	winefs_logentry_t *le = (winefs_logentry_t *)(journal_vaddr + jtail);
	void *start = le;

	winefs_memunlock_range(sb, start, jsize - jtail);
	while (jtail < jsize) {
		invalidate_gen_id(le);
		le++;
		jtail += LOGENTRY_SIZE;
	}
	winefs_memlock_range(sb, start, jsize - jtail);
}
 ...
 * the same cacheline */
static void winefs_forward_journal(struct super_block *sb, struct winefs_sb_info
		*sbi, winefs_journal_t *journal)
{
	...
	winefs_memlock_range(sb, journal, sizeof(*journal));
	winefs_flush_buffer(journal, sizeof(*journal), false);
}

static int winefs_recover_undo_journal(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_journal_t *journal;
	uint32_t tail;
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		while (head != tail) {
			...
			le = (winefs_logentry_t *)(sbi->journal_base_addr[i] + tail);
			if (gen_id == le16_to_cpu(le->gen_id)) {
				tail = winefs_recover_transaction(sb, head, tail, le);
			} else {
				if (gen_id == MAX_GEN_ID) {
					winefs_memunlock_range(sb, le, sizeof(*le));
					invalidate_gen_id(le);
		...
		winefs_forward_journal(sb, sbi, journal);
		PERSISTENT_MARK();
		PERSISTENT_BARRIER();
	}
	return 0;
...
static int winefs_recover_redo_journal(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_journal_t *journal;
	uint32_t tail;
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		while (head != tail) {
			le = (winefs_logentry_t *)(sbi->journal_base_addr + head);
			if (gen_id == le16_to_cpu(le->gen_id)) {
				head = winefs_process_transaction(sb, head, tail,
								le, true, &processed);
			} else {
				if (gen_id == MAX_GEN_ID) {
					winefs_memunlock_range(sb, le, sizeof(*le));
...
int winefs_recover_journal(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	...
	if (i < sbi->cpus) {
		if (sbi->redo_log)
			winefs_recover_redo_journal(sb);
		else
			winefs_recover_undo_journal(sb);
	}
