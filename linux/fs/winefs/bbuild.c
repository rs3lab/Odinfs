/*
 * WINEFS emulated persistence. This file contains code to 
...
static inline void set_bm(unsigned long bit, struct scan_bitmap *bm,
	enum bm_type type)
{
	switch (type) {
	case BM_4K:
...
static void winefs_clear_datablock_inode(struct super_block *sb)
{
	struct winefs_inode *pi =  winefs_get_inode(sb, WINEFS_BLOCKNODE_IN0);
...
static void winefs_clear_inodelist_inode(struct super_block *sb)
{
	struct winefs_inode *pi =  winefs_get_inode(sb, WINEFS_INODELIST_IN0);
	winefs_transaction_t *trans;
	__le64 root = pi->root;
	unsigned int height = pi->height;
	unsigned int btype = pi->i_blk_type;
...
static int winefs_failure_insert_inodetree(struct super_block *sb,
	unsigned long ino_low, unsigned long ino_high)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct inode_map *inode_map;
	struct winefs_range_node *prev = NULL, *next = NULL;
	...
	if (ino_low > ino_high) {
		winefs_err(sb, "%s: ino low %lu, ino high %lu\n",
	...
	ret = winefs_find_free_slot(tree, internal_low, internal_high,
				  &prev, &next);
	if (ret) {
		winefs_dbg("%s: ino %lu - %lu already exists!: %d\n",
	...
	if (prev && next && (internal_low == prev->range_high + 1) &&
		...
		winefs_free_inode_node(sb, next);
		goto finish;
	}
	...
	new_node = winefs_alloc_inode_node(sb);
	WINEFS_ASSERT(new_node);
	new_node->range_low = internal_low;
	new_node->range_high = internal_high;
	ret = winefs_insert_inodetree(sbi, new_node, cpu);
	if (ret) {
		winefs_err(sb, "%s failed\n", __func__);
		winefs_free_inode_node(sb, new_node);
		goto finish;
	}
	inode_map->num_range_node_inode++;
...
static void winefs_destroy_blocknode_tree(struct super_block *sb, int cpu)
{
	struct free_list *free_list;

	free_list = winefs_get_free_list(sb, cpu);
	winefs_destroy_range_node_tree(sb, &free_list->unaligned_block_free_tree);
	winefs_destroy_range_node_tree(sb, &free_list->huge_aligned_block_free_tree);
}

static void winefs_destroy_blocknode_trees(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	int i;

	for (i = 0; i < sbi->cpus; i++) {
		winefs_destroy_blocknode_tree(sb, i);
	}
}

static int winefs_init_blockmap_from_inode(struct super_block *sb)
{
	struct free_list *free_list;
	struct winefs_range_node_lowhigh *entry;
	struct winefs_range_node *blknode;
	u64 curr_p;
	u64 cpuid;
	...
	winefs_dbg("total_blocknodes = %lu, num_blocks = %lu, num_free_blocks = %lu, blocknode mapping file size = %llu\n",
		 total_blocknodes, sbi->num_blocks, sbi->num_free_blocks, pi->i_size);

	for (block_ctr = 0; block_ctr < pi->i_size / sbi->blocksize; block_ctr++) {
		__winefs_find_data_blocks(sb, pi, block_ctr, &curr_p, 1);
		...
		for (node_ctr = 0; node_ctr < RANGENODE_PER_PAGE; node_ctr++) {
			...
			entry = (struct winefs_range_node_lowhigh *)winefs_get_block(sb, curr_p);
			blknode = winefs_alloc_blocknode(sb);
			...
			if (blknode->range_low == 0 && blknode->range_high == 0) {
				winefs_free_blocknode(sb, blknode);
				goto next;
			}
			...
			free_list = winefs_get_free_list(sb, cpuid);
			...
			if (blknode->range_low % 512 == 0 &&
				...
				ret = winefs_insert_blocktree(&free_list->huge_aligned_block_free_tree, blknode);
				free_list->num_blocknode_huge_aligned++;
				if (free_list->num_blocknode_huge_aligned == 1) {
					free_list->first_node_huge_aligned = blknode;
				}
			} else {
				ret = winefs_insert_blocktree(&free_list->unaligned_block_free_tree, blknode);
			...
			if (ret) {
				...
				winefs_free_blocknode(sb, blknode);
				WINEFS_ASSERT(0);
				winefs_destroy_blocknode_trees(sb);
				goto out;
			}
			free_list->num_free_blocks += blknode->range_high - blknode->range_low + 1;
		next:
			curr_p += sizeof(struct winefs_range_node_lowhigh);
			blocknode_ctr++;
		}
	}
out:
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		/*
		 * winefs_dbg() causes segfault when uncommented, when first_node is NULL
		 */
		/*
		winefs_dbg("%s: free list %d: block start %lu, end %lu, "
...
static void winefs_destroy_inode_trees(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct inode_map *inode_map;
	int i;
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		winefs_destroy_range_node_tree(sb,
					     &inode_map->inode_inuse_tree);
	}
}

static int winefs_init_inode_list_from_inode(struct super_block *sb)
{
	struct winefs_range_node_lowhigh *entry;
	struct winefs_range_node *range_node, *new_range_node;;
	...
	for (block_ctr = 0; block_ctr < pi->i_size / sbi->blocksize; block_ctr++) {
		...
		for (inode_node_ctr = 0; inode_node_ctr < RANGENODE_PER_PAGE; inode_node_ctr++) {
			...
			if (cpuid >= sbi->cpus) {
				winefs_err(sb, "Invalid cpuid %lu\n", cpuid);
				winefs_free_inode_node(sb, range_node);
				WINEFS_ASSERT(0);
				winefs_destroy_inode_trees(sb);
				goto out;
			}
			...
			/* winefs_dbg_verbose("%s: inode_node_ctr = %u, block_ctr = %lu, pi->i_size = %lu, range_node->range_low = %lu, range_node->range_high = %lu, total_inode_nodes = %Llu\n", */
			/* 		   __func__, inode_node_ctr, block_ctr, */
			/* 		   pi->i_size, */
			/* 		   range_node->range_low & ~CPUID_MASK, */
			/* 		   range_node->range_high, total_inode_nodes); */
			ret = winefs_insert_inodetree(sbi, range_node, cpuid);
			if (ret) {
				winefs_err(sb, "%s failed\n", __func__);
				winefs_free_inode_node(sb, range_node);
				WINEFS_ASSERT(0);
			...
			if (!inode_map->first_inode_range) {
				/* new_range_node = winefs_alloc_inode_node(sb); */
				/* if (new_range_node == NULL) */
				/* 	WINEFS_ASSERT(0); */
				/* new_range_node->range_low = 0; */
				/* new_range_node->range_high = WINEFS_NORMAL_INODE_START / sbi->cpus; */
				/* winefs_dbg("inserted range node with range_low = %lu, " */
				/* 	 "range_high = %lu in inode_map first_inode_range " */
				/* 	 "with CPUID = %d\n", new_range_node->range_low, */
				/* 	 new_range_node->range_high, cpuid); */
				/* winefs_insert_inodetree(sbi, new_range_node, cpuid); */
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		if (!inode_map->first_inode_range) {
			...
			ret = winefs_insert_inodetree(sbi, new_range_node, i);
			if (ret) {
				winefs_err(sb, "%s failed\n", __func__);
				winefs_free_inode_node(sb, new_range_node);
				WINEFS_ASSERT(0);
				winefs_destroy_inode_trees(sb);
				goto out;
			}
		...
		winefs_dbg("inode_map->first_inode_range->range_low = %lu, "
			   "inode_map->first_inode_range->range_high = %lu\n",
			   inode_map->first_inode_range->range_low,
			   inode_map->first_inode_range->range_high);
	}
	winefs_dbg("%s: s_inodes_used_count = %u\n",
		   __func__, sbi->s_inodes_used_count);
out:
	return ret;
}

/* static bool winefs_can_skip_full_scan(struct super_block *sb) */
/* { */
/* 	struct winefs_inode *pi =  winefs_get_inode(sb, WINEFS_BLOCKNODE_IN0); */
/* 	struct winefs_super_block *super = winefs_get_super(sb); */
/* 	struct winefs_sb_info *sbi = WINEFS_SB(sb); */
/* 	__le64 root; */
...
/* 	winefs_init_blockmap_from_inode(sb); */

/* 	root = pi->root; */
/* 	height = pi->height; */
/* 	btype = pi->i_blk_type; */
/* 	/1* pi->i_size can not be zero *1/ */
/* 	last_blocknr = (le64_to_cpu(pi->i_size) - 1) >> */
/* 					winefs_inode_blk_shift(pi); */

/* 	/1* Clearing the datablock inode *1/ */
/* 	winefs_clear_datablock_inode(sb); */
...
static int winefs_allocate_datablock_block_inode(winefs_transaction_t *trans,
	struct super_block *sb, struct winefs_inode *pi, unsigned long num_blocks)
{
	int errval;
	...
	pi->i_blk_type = WINEFS_BLOCK_TYPE_4K;
	pi->i_flags = 0;
	pi->height = 0;
	pi->i_dtime = 0;
	...
	errval = __winefs_alloc_blocks_wrapper(trans, sb, pi, 0,
					     num_blocks, false, 0, 0);
	return errval;
}

static u64 winefs_append_range_node_entry(struct super_block *sb,
	struct winefs_range_node *curr, void *p, unsigned long cpuid)
{
	/* size_t size = sizeof(struct winefs_range_node_lowhigh); */
	struct winefs_range_node_lowhigh *entry;

	entry = p;

	//winefs_memunlock_range(sb, entry, size);
	...
	//winefs_memlock_range(sb, entry, size);
	winefs_dbg_verbose("append entry block low 0x%lx, high 0x%lx\n",
		 curr->range_low, curr->range_high);

	winefs_flush_buffer(entry, sizeof(struct winefs_range_node_lowhigh), 0);
	return 0;
}

static u64 winefs_save_range_nodes(struct super_block *sb, struct winefs_inode *pi,
	struct rb_root *tree, u64 blocknode_ctr, unsigned long cpuid)
{
	...
	if (blocknode_ctr != 0) {
		...
		__winefs_find_data_blocks(sb, pi, blocknr, &bp, 1);
		p = (unsigned long)winefs_get_block(sb, bp);
	...
	while (temp) {
		...
		winefs_append_range_node_entry(sb, curr,
					     (void *)p, cpuid);
		p += (unsigned long)size;
		temp = rb_next(temp);
		rb_erase(&curr->node, tree);
		winefs_free_range_node(curr);
		blocknode_ctr++;
	}
...
static u64 winefs_save_free_list_blocknodes(struct super_block *sb, int cpu,
					  struct winefs_inode *pi, u64 blocknode_ctr)
{
	struct free_list *free_list;

	free_list = winefs_get_free_list(sb, cpu);
	blocknode_ctr = winefs_save_range_nodes(sb, pi,
...
void winefs_save_blocknode_mappings(struct super_block *sb)
{
	...
	winefs_dbg("%s: num blocknodes to save = %lu. num pages = %lu, num_blocknode_allocated = %lu\n",
		 __func__, num_blocknode, num_pages, sbi->num_blocknode_allocated);

	/*
	num_blocks = ((sbi->num_blocknode_allocated * sizeof(struct
		winefs_range_node_lowhigh) - 1) >> sb->s_blocksize_bits) + 1;
	*/
	/* 2 log entry for inode, 2 lentry for super-block */
	trans = winefs_new_transaction(sb, MAX_INODE_LENTRIES + MAX_METABLOCK_LENTRIES + MAX_SB_LENTRIES, 0);
	if (IS_ERR(trans))
		return;

	winefs_add_logentry(sb, trans, pi, MAX_DATA_PER_LENTRY, LE_DATA);

	errval = winefs_allocate_datablock_block_inode(trans, sb, pi, num_pages);
	pi->winefs_ino = WINEFS_BLOCKNODE_IN0;

	if (errval != 0) {
		winefs_dbg("Error saving the blocknode mappings: %d\n", errval);
		winefs_abort_transaction(sb, trans);
		return;
	}

	for (i = 0; i < sbi->cpus; i++) {
		blocknode_ctr = winefs_save_free_list_blocknodes(sb, i, pi, blocknode_ctr);
	}
	...
	winefs_add_logentry(sb, trans, &super->s_wtime,
			WINEFS_FAST_MOUNT_FIELD_SIZE, LE_DATA);

	winefs_memunlock_range(sb, &super->s_wtime, WINEFS_FAST_MOUNT_FIELD_SIZE);
	...
	winefs_memlock_range(sb, &super->s_wtime, WINEFS_FAST_MOUNT_FIELD_SIZE);
	/* commit the transaction */
	winefs_commit_transaction(sb, trans);
}
...
void winefs_save_inode_list(struct super_block *sb)
{
	...
	winefs_dbg_verbose("%s: num inode nodes = %lu. num pages = %lu\n",
	...
	*/
	/* 2 log entry for inode, 2 lentry for super-block */
	trans = winefs_new_transaction(sb, MAX_INODE_LENTRIES + MAX_SB_LENTRIES, 0);
	if (IS_ERR(trans))
		return;
	...
	errval = winefs_allocate_datablock_block_inode(trans, sb, pi, num_pages);
	pi->winefs_ino = WINEFS_INODELIST_IN0;
...
static int winefs_insert_blocknode_map(struct super_block *sb,
	int cpuid, unsigned long low, unsigned long high)
{
	struct free_list *free_list;
	struct rb_root *tree;
	struct winefs_range_node *blknode = NULL;
	unsigned long num_blocks = 0;
	int ret;

	num_blocks = high - low + 1;
	winefs_dbg_verbose("%s: cpu %d, low %lu, high %lu, num %lu\n",
	...
	ret = winefs_insert_blocktree(tree, blknode);
	if (ret) {
		winefs_err(sb, "%s failed\n", __func__);
		winefs_free_blocknode(sb, blknode);
		goto out;
...
static int __winefs_build_blocknode_map(struct super_block *sb,
	unsigned long *bitmap, unsigned long bsize, unsigned long scale)
{
	...
	*/
	...
	while (1) {
		...
		if (winefs_insert_blocknode_map(sb, cpuid,
				low << scale, (next << scale) - 1)) {
			winefs_dbg("Error: could not insert %lu - %lu\n",
				low << scale, ((next << scale) - 1));
		}
		start = next;
		if (next == bsize)
...
static void winefs_update_4K_map(struct super_block *sb,
...
static struct scan_bitmap *global_bm[MAX_CPUS];

static int winefs_build_blocknode_map(struct super_block *sb,
	unsigned long initsize, unsigned long initsize_2)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		bm = global_bm[i];
		winefs_update_4K_map(sb, bm, bm->scan_bm_2M.bitmap,
			bm->scan_bm_2M.bitmap_size * 8, PAGE_SHIFT_2M - 12);
		winefs_update_4K_map(sb, bm, bm->scan_bm_1G.bitmap,
	...
	ret = __winefs_build_blocknode_map(sb, final_bm->scan_bm_4K.bitmap,
...
static wait_queue_head_t finish_wq;
static int *finished;

static void winefs_inode_crawl_recursive(struct super_block *sb,
				...
				u32 height, u8 btype)
{
	...
	node = winefs_get_block(sb, block);
	set_bit(block >> PAGE_SHIFT, bm->scan_bm_4K.bitmap);
	for (i = 0; i < (1 << META_BLK_SHIFT); i++) {
		if (node[i] == 0)
			continue;
		winefs_inode_crawl_recursive(sb, bm,
			le64_to_cpu(node[i]), height - 1, btype);
	}
}

static inline void winefs_inode_crawl(struct super_block *sb,
				    struct scan_bitmap *bm,
				    ...
				    struct task_ring *ring)
{
	...
	winefs_inode_crawl_recursive(sb, bm, le64_to_cpu(pi->root), pi->height,
					pi->i_blk_type);
}

/* static void winefs_inode_table_crawl_recursive(struct super_block *sb, */
/* 				struct scan_bitmap *bm, unsigned long block, */
...
/* 		if (likely(btype == WINEFS_BLOCK_TYPE_2M)) */
/* 			set_bit(block >> PAGE_SHIFT_2M, bm->scan_bm_2M.bitmap); */
/* 		else */
/* 			set_bit(block >> PAGE_SHIFT, bm->scan_bm_4K.bitmap); */
...
/* 			pi = (struct winefs_inode *)((void *)node + */
/*                                                         WINEFS_INODE_SIZE * i); */
...
/* 		if (node[i] == 0) */
/* 			continue; */
/* 		winefs_inode_table_crawl_recursive(sb, bm, */
/* 			le64_to_cpu(node[i]), height - 1, btype); */
/* 	} */
/* #endif */
/* } */
...
static inline int winefs_failure_update_inodetree(struct super_block *sb,
	struct winefs_inode *pi, unsigned long *ino_low, unsigned long *ino_high)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);

	if (*ino_low == 0) {
		*ino_low = *ino_high = pi->winefs_ino;
	} else {
		if (pi->winefs_ino == *ino_high + sbi->cpus) {
			*ino_high = pi->winefs_ino;
		} else {
			/* A new start */
			winefs_failure_insert_inodetree(sb, *ino_low, *ino_high);
			*ino_low = *ino_high = pi->winefs_ino;
		}
	}
...
static int failure_thread_func(void *data)
{
	...
	pi = winefs_get_inode_table(sb);
	data_bits = winefs_blk_type_to_shift[pi->i_blk_type];
	num_inodes_per_page = 1 << (data_bits - WINEFS_INODE_BITS);

	ring = &task_rings[cpuid];
	winefs_init_header(sb, &sih, 0);
	...
	for (count = 0; count < ring->num; count++) {
		...
		for (i = 0; i < num_inodes_per_page; i++) {
			pi_addr = curr + i * WINEFS_INODE_SIZE;
			ret = winefs_get_reference(sb, pi_addr, &fake_pi,
						 (void **)&pi,
						 sizeof(struct winefs_inode));
			if (ret) {
				winefs_dbg("Recover pi @ 0x%llx failed\n",
					 pi_addr);
				continue;
			}

			if (pi->winefs_ino < WINEFS_NORMAL_INODE_START)
				continue;
			...
			winefs_dbg_verbose("%s: Valid inode. "
					 "Now crawling. ino = %llu\n",
					 __func__, pi->winefs_ino);

			winefs_inode_crawl(sb, global_bm[cpuid], pi, ring);
			winefs_failure_update_inodetree(sb, pi,
						      &ino_low, &ino_high);
...
static int winefs_failure_recovery_crawl(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct winefs_inode_info_header sih;
	...
	pi = winefs_get_inode(sb, WINEFS_ROOT_INO);
	...
	for (cpuid = 0; cpuid < sbi->cpus; cpuid++) {
		...
		while (curr) {
			if (ring->num >= 512) {
				winefs_err(sb, "%s: ring size too small\n",
					 __func__);
				return -EINVAL;
			}
			...
			curr_addr = (unsigned long)winefs_get_block(sb,
								  curr);
	...
	winefs_inode_crawl(sb, global_bm[0], pi, ring);
	return ret;
}

int winefs_failure_recovery(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct task_ring *ring;
	int ret;
	...
	ret = winefs_failure_recovery_crawl(sb);
	...
	free_resources(sb);
	winefs_dbg("Failure recovery total recovered %u\n",
...
static bool winefs_try_normal_recovery(struct super_block *sb)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct winefs_inode *pi =  winefs_get_inode(sb, WINEFS_BLOCKNODE_IN0);
	struct winefs_super_block *super = winefs_get_super(sb);
	...
	ret = winefs_init_blockmap_from_inode(sb);

	if (ret) {
		winefs_err(sb, "init blockmap failed, fall back to failure recovery\n");
		return false;
	}
	...
	ret = winefs_init_inode_list_from_inode(sb);
	if (ret) {
		winefs_err(sb, "init inode list failed, fall back to failure recovery\n");
		winefs_destroy_blocknode_trees(sb);
		return false;
	}
	/* Clearing the datablock inode */
	winefs_clear_inodelist_inode(sb);
 ...
 */
int winefs_recovery(struct super_block *sb, size_t size, size_t size_2)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct winefs_super_block *super = winefs_get_super(sb);
	...
	journal_data_start = (INODE_TABLE0_START + INODE_TABLE_NUM_BLOCKS) *
		sbi->blocksize;
	winefs_init_blockmap(sb, journal_data_start + (sbi->cpus*sbi->jsize), 1);

	value = winefs_try_normal_recovery(sb);
	if (value) {
		winefs_dbg("WINEFS: Normal shutdown\n");
	} else {
		winefs_dbg("WINEFS: Failure recovery\n");
		ret = alloc_bm(sb, initsize, initsize_2);
		if (ret)
			goto out;

		sbi->s_inodes_used_count = 0;
		ret = winefs_failure_recovery(sb);
		if (ret)
			goto out;

		ret = winefs_build_blocknode_map(sb, initsize, initsize_2);
	}
...
out:
	...
	if (sbi->s_inodes_used_count < WINEFS_NORMAL_INODE_START)
		sbi->s_inodes_used_count = WINEFS_NORMAL_INODE_START;
