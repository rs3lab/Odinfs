...
#define FILES_CREATE_PER_SEC_PER_CORE (550)
#define MAX_NUM_OF_CORES (224)
...
static int pre_work(struct worker *worker)
{
    ...
    create_files = FILES_CREATE_PER_SEC_PER_CORE * MAX_NUM_OF_CORES * 
        bench->duration;


    /* time to create files */
    for (i = 0; i < (create_files / (max_id + 1)); 
            ++i, ++worker->private[0])
    {
