...
class PerfMon(object):
    ...
    def start(self):
        ...
        if self.LEVEL == PerfMon.LEVEL_PERF_C2C:
            self._perf_c2c_record_start()
        if self.LEVEL == PerfMon.LEVEL_PMWATCH:
            self._perf_pmwatch_start()
    ...
    def stop(self):
        try:
            ...
            if self.LEVEL >= PerfMon.LEVEL_LOW:
                self._cpu_stat_stop()
            if self.LEVEL == PerfMon.LEVEL_PERF_C2C:
                self._perf_c2c_record_stop()
            if self.LEVEL == PerfMon.LEVEL_PMWATCH:
                self._perf_pmwatch_stop()
    ...
    def _perf_c2c_record_start(self):
        perf_out = os.path.normpath(
            os.path.join(self.DIR, "%s.perf.data" % self.FILE))
        self._exec_cmd("sudo perf c2c record -F %s -a -g -o %s &" %
                       (PerfMon.PERF_SAMPLE_RATE, perf_out))

    def _perf_c2c_record_stop(self):
        self._perf_stop("pmwatch")

    def _perf_pmwatch_start(self):
        perf_out = os.path.normpath(
            os.path.join(self.DIR, "%s.pmwatch.csv" % self.FILE))
        self._exec_cmd("sudo pmwatch %s -g -f %s &" %
                       (PerfMon.PMWATCH_SAMPLE_INTERVAL, perf_out))

    def _perf_stop(self, proc):
        with open("/dev/null", "a") as fd:
            self._exec_cmd("sudo kill -INT $(pgrep %s)" % proc, fd)
